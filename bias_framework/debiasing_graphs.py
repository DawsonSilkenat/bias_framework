...
class DebiasingGraphsObject:
    ...
    def get_single_graph(self, error_metric: str, fairness_metric: str, showlegend: bool = True, include_fairea_labels: bool = True) -> go.Figure:
        ...
        figure.update_layout(
            title=f"Debias Methodologies impact on {error_metric} and {fairness_metric}")
            ...
            for col, fairness_metric in enumerate(fairness_metrics):
                ...
                if col == 0:
                    subplots.update_yaxes(
                        title_text=f"Performance ({error_metric})", col=col+1, row=row+1)
                if row == len(error_metrics) - 1:
                    subplots.update_xaxes(
                        title_text=f"Bias ({fairness_metric})", col=col+1, row=row+1)
    ...
    def __create_scatter_with_baseline(self, error_metric: str, fairness_metric: str, debias_methodologies: list[str] = None, showlegend=True, include_fairea_labels=True) -> list[go.Scatter]:
        ...
        for i in range(len(self.baseline_curve)):
            baseline_curves_figures.append(self.baseline_curve[i].get_baseline_curve(
                error_metric, fairness_metric, colors[curve_color_indexes[i]], showlegend, include_fairea_labels))
