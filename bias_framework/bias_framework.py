...
class Bias_Framework:
    def __init__(self, model, df_x_train: pd.DataFrame, df_x_validation: pd.DataFrame, df_y_train: pd.DataFrame,
                 df_y_validation: pd.DataFrame, pre_processing=None, **model_args) -> None:
        ...
        Args:
            model: The ML model to which the bias framework will be applied. This model must have a fit method and predict method.
                I am assuming at the moment that this will be an sklearn ml model, might try to modify this to be more flexible later.
        ...
        # Not all models implement a function for prediction probability. Probabilities are required for some postprocessing debiasing.
        # The CalibratedClassifierCV method preforms probability calibration, which also adds this functionality if it is not part of the model
    ...
    def set_privileged_combinations(self, privileged_combinations: list[dict[str: any]]) -> None:
        ...
        Args:
            privileged_combinations: a list of dictionaries, each dictionary maps a set of columns name to the values they must take.
                Note: Each columns name must map to a single value, not a list of possible values. The values of any column names not included in the dictionary are ignored
    ...
    def set_unprivileged_combinations(self, unprivileged_combinations: list[dict[str: any]]) -> None:
        ...
        Args:
            unprivileged_combinations: a list of dictionaries, each dictionary maps a set of columns name to the values they must take.
                Note: Each columns name must map to a single value, not a list of possible values. The values of any column names not included in the dictionary are ignored
    ...
    def run_framework(self):
        """Executes the framework using the model, data, and pre-processing provided at initialisation and the definition of privilege set using one of the provided methods.
            This will run through a number of debiasing methodologies and save the results. This may take some time.
        ...
        train_true_labels, train_predictions, validation_predictions, validation_to_predict = self.__get_aif360_datasets(
            x_train, x_validation, training_predicted_values, training_probabilities, validation_predicted_values,
    ...
    def __get_aif360_datasets(self, x_train, x_validation, training_predicted_values, training_probabilities,
        ...
        if isinstance(x_train, pd.DataFrame):
            ...
            raise RuntimeError(
                f"Pre-processing results in an unrecognised datatype. "
                f"Please make sure running pre-processing returns a pandas dataframe or a numpy array. "
                f"If you have used no pre-processing, make sure your data is of the expected type\nEncountered type: {type(x_train)}")
        ...
        # Some debiasing methodologies need a dataset not only for training but also application.
        # This dataset exists for that purpose, and hides class labels so that the information cannot leak
    ...
    def __no_debiasing(self, x_train, x_validation):
        ...
        self.__metrics_by_debiasing_technique["no debiasing"] = bootstrap_all_metrics(self.y_validation,
        ...
        # Most debiasing analysis functions won't return anything, however having default results is needed for
        # fairea and postprocessing debiasing
    ...
    # def __optimized_preprocessing(self):
        # this looks like it requires more knowledge of the dataset than I  expect this framework to know at this point in time,
        # so leaving this unimplemented Refer back to these when it comes time to implement
        # from aif360.algorithms.preprocessing import OptimPreproc
        # from aif360.algorithms.preprocessing.optim_preproc_helpers.data_preproc_functions import load_preproc_data_adult
        pass

    def __learning_fair_representation(self, train_true_labels, validation_to_predict):
        # Only running one number_of_prototypes due to runtime. When ran with [5, 10] this took over an hour while
        # other methods took minutes.
        for number_of_prototypes in [5]:
            ...
            fair_representation = LFR(unprivileged_groups=[{"Is Privileged": 0}],
                                      privileged_groups=[{"Is Privileged": 1}], k=number_of_prototypes)

            transformed_data = fair_representation.fit_transform(training_dataset)

            # This debiasing methodology also mutates the labels. If there is a single label,the classifier may raise
            # an exception, so this case must be handled separately
            ...
            if len(classes) == 1:
                ...
                # Note that this debiasing methodology also seems to update the class labels.
                # I'm not entirely clear on why, but does seem to get better results with the updated labels.
            ...
            self.__metrics_by_debiasing_technique[
                f"learning fair representation with {number_of_prototypes} prototypes"] = bootstrap_all_metrics(
                self.y_validation, predicted_values, self.privilege_validation)
            self.__metrics_by_debiasing_technique[
                f"learning fair representation with {number_of_prototypes} prototypes"]["raw"] = predicted_values
    ...
    def __reweighing(self, train_true_labels, validation_to_predict):
        ...
        self.model.fit(transformed_data.features, transformed_data.labels.ravel(),
                       sample_weight=transformed_data.instance_weights, **self.__model_args)
        ...
        self.__metrics_by_debiasing_technique["reweighing"] = bootstrap_all_metrics(self.y_validation, predicted_values,
                                                                                    self.privilege_validation)
    ...
    def __reject_option_classification(self, train_true_labels, train_predictions, validation_predicted_values):
        ...
        for metric_name in ["Statistical parity difference", "Average odds difference", "Equal opportunity difference"]:
            ...
            reject_option_classification = RejectOptionClassification([{"Is Privileged": 0}],
            ...
            self.__metrics_by_debiasing_technique[
                f"reject option classification {metric_name.lower()} optimised"] = bootstrap_all_metrics(
                self.y_validation, predicted_values, self.privilege_validation)
            self.__metrics_by_debiasing_technique[f"reject option classification {metric_name.lower()} optimised"][
    ...
    def __calibrated_equal_odds(self, train_true_labels, train_predictions, validation_predicted_values):
        for cost_constraint in ["fpr", "fnr", "weighted"]:
            ...
            calibrated_equal_odds = CalibratedEqOddsPostprocessing([{"Is Privileged": 0}],
                                                                   privileged_groups=[{"Is Privileged": 1}],
            ...
            self.__metrics_by_debiasing_technique[
                f"calibrated equal odds using {cost_constraint} cost"] = bootstrap_all_metrics(self.y_validation,
                                                                                               predicted_values,
                                                                                               self.privilege_validation)
            self.__metrics_by_debiasing_technique[f"calibrated equal odds using {cost_constraint} cost"][
                "raw"] = predicted_values
    ...
    def __equal_odds(self, train_true_labels, train_predictions, validation_predicted_values):
        ...
        self.__metrics_by_debiasing_technique["equal odds"] = bootstrap_all_metrics(self.y_validation, predicted_values,
                                                                                    self.privilege_validation)
