...
class FaireaCurve(Baseline):
    ...
    def __init__(self, true_values: np.array, predicted_values: np.array, privilege_status: np.array, fractions_to_mutate: list[float] = [i/10 for i in range(11)], repetitions: int = 50) -> None:
        ...
        self.metrics_by_mutation = _fairea_model_mutation(
            true_values, predicted_values, privilege_status, fractions_to_mutate, repetitions)
    ...
    def get_baseline_curve(self, error_metric: str, fairness_metric: str, color: str, showlegend: bool = True, include_labels: bool = True) -> go.Scatter:
        ...
        Args:
            error_metric (str): Which measurement of error is used for the curve
            fairness_metric (str): Which measurement of bias is used for the curve
            color (str): The color to be used in the resulting curve
            showlegend (bool, optional): If the curve should be included in the legend of a graph using this curve. Defaults to True.
            include_labels (bool, optional): Whether the points along the curve should be labelled with F_n, where n is the percentage mutated. Defaults to True.
        Returns:
            go.Scatter: A plotly scatter object representing the specified curve
        """
        fairea_labels = []
        fairea_x = []
        fairea_y = []
...
def _fairea_model_mutation(true_values: np.array, predicted_values: np.array, privilege_status: np.array, fractions_to_mutate: list[float], repetitions: int, seed: int = None) -> list[tuple[float, dict[str, dict[str, float]]]]:
    ...
    # The first float is the mutation fraction, the dictionary are the metric results for that
