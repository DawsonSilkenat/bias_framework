...
def covert_to_datasets_train(x_train: ArrayLike, y_train: np.array, train_predictions: np.array, train_probabilities: np.array, privilege_train: np.array) -> tuple[StandardDataset, StandardDataset]:
    ...
    Args:
        ...
        privilege_train (np.array): Binary array, the ith entry of which is a 1 if the ith element of the training data belongs to the privileged group, and 0 otherwise

    Returns:
        tuple[StandardDataset, StandardDataset]: 
            The first dataset (ds_train_true_labels) contains the training data, privilege statuses, and true classes
            The second dataset (ds_train_predictions) contains the training data, privilege statuses, probability of belonging to the privileged class, and predicted class label
    """
